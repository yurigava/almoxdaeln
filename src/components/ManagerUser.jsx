...
export default class ManagerUser extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    //this.props.allUsers.forEach(function (campo) {
      //alert("usuario: " + campo.user + " role: " + campo.role);
    //});
  }
  ...
  handleNewEquipment(event) {
    if(this.props.allUsers.length === 0 ? false : this.props.allUsers[this.props.allUsers.length-1].user === null ? true : this.props.allUsers[this.props.allUsers.length-1].role === null ? true : false) {
      return
    }
  ...
  handleFormSubmit(event) {
    ...
    for(var i=0; i<this.props.allUsers.length ; i++) {
      for(var j=0; j<this.props.allUsers.length ; j++) {
        //alert("i: " + i + " j: " + j);
        if(this.props.allUsers[i].user === this.props.allUsers[j].user & i !== j) {
          this.props.setError("Usuário(a) '" + this.props.allUsers[j].user + "' duplicado(a). Não foi possível enviar os dados");
          this.props.setIsYesNoMessage(false);
          //duplicado = true;
          return;
        }
        else if(this.props.allUsers[i].user === this.props.usuario) {
          this.props.setError("Usuário(a) '" + this.props.usuario + "' já adicionado(a). Não foi possível enviar os dados");
          this.props.setIsYesNoMessage(false);
          return;
        }
      }
    ...
    //if (duplicado === true) {
    //  this.props.setError("Usuário duplicado. Não foi possível enviar os dados");
    //  this.props.setIsYesNoMessage(false);
    //  return;
    //}
    
    this.props.allUsers.forEach(campo => {
      if(campo.user !== this.props.usuario) {
        userRoles.push({
          user: campo.user,
          role: campo.role
        });
    ...
    //userRoles.forEach(campo => {
      //alert("user: " + campo.user + " role: " + campo.role);
    //});
    this.props.insertUserRole(userRoles, this.props.usuario);
  ...
  handleCloseDialog() {
    //alert("isdata: " + this.props.isDataSubmitted);
          ...
          {this.props.allUsers.map((usuario, index) => (
            <Row bottom="xs" around="xs" center="xs" key={index} >
              <Col xs={11} sm={11} md={11} >
                {usuario.user !== null & usuario.role !== null & this.state.flagUser !== index ?
                  <div>
                    <Row bottom="xs" around="xs" center="xs" key={index} >
                      <Col xs={11} sm={11} md={11} >
                        <Paper
                        ...
                        >
                          <text style={{marginLeft: '15.5px'}}>
                            {usuario.user + " - " + this.findRegra(usuario.role)}
                          </text>
                        </Paper>
                      </Col>
                    </Row>
          ...
          <Row bottom="xs" center="xs" style={{height: '60px'}} >
            <Col>
              <RaisedButton
                ...
                type="button"
                label="Adicionar"
                disabled={this.props.allUsers.length === 0 ? false : this.props.allUsers[this.props.allUsers.length-1].user === null ? true : this.props.allUsers[this.props.allUsers.length-1].role === null ? true : false}
                primary={false}
