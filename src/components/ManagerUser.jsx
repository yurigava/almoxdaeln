...
export default class ManagerUser extends React.Component {
  constructor(props) {
    ...
    this.handleChangeRegra = this.handleChangeRegra.bind(this);
  ...
  handleRemoveEquipment(name, event) {
    if(this.props.allUser.length <= 1) {
  ...
  handleNewEquipment(event) {
    if(this.props.allUser[this.props.allUser.length-1].role === null) {
      return
    }
    //this.props.addEquip();
  }
  ...
  render () {
    ...
    return (
      <div>
        ...
        <Grid fluid >
          {this.props.allUser.map((usuario, index) => (
          ...
          <Row bottom="xs" center="xs" style={{height: '60px'}} >
            <Col>
              <RaisedButton
                ...
                type="button"
                label="Adicionar"
                disabled={this.props.allUser[this.props.allUser.length-1].role === null ? true : false}
                primary={false}
...
ManagerUser.propTypes = {
  ...
  insertUsers: PropTypes.func.isRequired,
};
