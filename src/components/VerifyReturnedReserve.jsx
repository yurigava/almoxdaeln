...
export default class VerifyReturnedReserve extends React.Component {
  ...
  handleAcceptAll() {
    const missingEquips = this.props.lentEquips.map(lentEquip => findNotContainedAinB(lentEquip.equips,
        this.state.readEquipsByTipo[lentEquip.name].map(equip => Number(equip))));
    this.props.setIsYesNoMessage(true);
    if(missingElements.filter(elem => elem.length > 0) > 0)
    ...
    this.props.lentEquips.forEach(lentEquip => {
      if(this.state.readEquipsByTipo[lentEquip.name] === undefined)
        update(equipsToRegister, {$push: lentEquip.equips});
      else
        update(equipsToRegister, {$push: this.state.readEquipsByTipo[lentEquip.name].map(equip => Number(equip))});
    });
    this.setState(update(this.state, {
      callbackOnYesDialog: {$set: () => this.registerReturnedEquips(reserveId, equipsToRegister)}
    }));
  }
    ...
    return (
      <div>
        ...
        <InformationTable
          ...
          callbackOnYesDialog={this.callbackOnYesDialog}
