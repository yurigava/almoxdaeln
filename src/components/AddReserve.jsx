...
const getMenuQuantidade = (quantidade) => {
  var vetor = [];
...
export default class AddReserve extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      equipReservados: [
        {index: 0, name: "equip0", value: "", errorText: "", familia: 0, tipo: 0, isMissingTipo: true, isMissingFamilia: true, isInputDisabled: false, maxQuantidade: ""}
      ],
      timeReserve: "",
      dateReserve: "",
      materia: "",
      indexinfoNumber: 0,
      quantidadeMax: this.props.quantidade,
      menuQuantidade: [],
      indexKey: ""      
    }
  ...
  handleChangeQuantidade(event, name, value) {
    //console.log(Object.keys(event));
    alert(value);
    //alert(event.currentTarget.name);
    //alert("props:" + this.props.quantidade);
    //const index = this.findEquipIndexQuantidade(this.props.quantidade, this.state.equipReservados);
  ...
  componentWillReceiveProps(nextProps) {   
    if(nextProps.quantidade !== this.props.quantidade) {
      ...
      //alert("index: " + index);
      let index = 0;
      const menuQuantidade = getMenuQuantidade(nextProps.quantidade);
      //alert("menu: " + menuQuantidade);
      this.setState(update(this.state, {
        menuQuantidade: { $set: menuQuantidade },
        equipReservados: {
          [index]: {
     ...
     //alert("menu: " + this.state.menuQuantidade + "max: " + this.state.equipReservados[0].maxQuantidade); 
    }
  }
  ...
  funcSetSelectedFamilia(name, familia) {
    ...
    this.props.quantidadeReserve(newValue);
  }
  ...
  findEquipIndexQuantidade(maxQuantidade, equipReservados) {
    return equipReservados.findIndex(equipReservado =>
      maxQuantidade == equipReservado.maxQuantidade
    )
  }
          ...
          {this.state.equipReservados.map((equipReservado) => (
            ...
            &nbsp;
              <Select
                ...
                onChange={this.handleChangeQuantidade.bind(this, 'oi')}
                disabled={equipReservado.isInputDisabled}
                ...
                //maxHeight={300}
              >
                {this.state.menuQuantidade.map((quant) => (
                  <MenuItem 
                    value={quant} 
                    primaryText={quant} 
                  />
