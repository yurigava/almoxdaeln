...
const getAllowedTipos = (quantidade) => {
  var vetor = [];
...
export default class AddReserve extends React.Component {
  ...
  handleChangeQuantidade(event, value) {
    this.setState(update(this.state, {
      quantidadeMax: {$set: value }
    }))
  }
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if(nextProps.quantidade !== this.props.quantidade) {
      //alert("depois");
      const allowedTipos = getAllowedTipos(nextProps.quantidade);
      //alert(allowedTipos);
      this.setState(update(this.state, {
        ...
        >
          ...
          {this.state.equipReservados.map((equipReservado) => (
            ...
            &nbsp;
              <DropDownMenu 
                maxHeight={300} 
                value={this.state.quantidadeMax} 
                onChange={this.handleChangeQuantidade}
                style={style}
              >
                {this.state.allowedTipos.map((quantidade) => (
                  <MenuItem key={quantidade} value={quantidade} primaryText={quantidade} />
                ))}
              </DropDownMenu>
