...
export default class AddReserve extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      equipReservados: [
        {index: 0, name: "equip0", value: "", errorText: "", familia: '', tipo: '', isMissingTipo: false, isMissingFamilia: false, isInputDisabled: false, maxQuantidade: 0}
      ],
      timeReserve: "",
      dateReserve: "",
      materia: "",
  ...
  handleFormSubmit(event) {
    ...
    console.log(this.state.equipReservados.length);
    for(var i=1; i<=this.state.equipReservados.length; i++) {
      if((this.state.equipReservados[i].familia !== undefined) && (this.state.equipReservados[i].value === "")) {
