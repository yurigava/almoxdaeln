...
export default class AddReserve extends React.Component {
  ...
  handleFormSubmit(event) {
    ...
    if(this.state.timeReserve === null || this.state.timeReserve === undefined || this.state.timeReserve === "" ) {
      this.props.setError("Por favor, insira horário da reserva.");
      return;
    }
    ...
    for(var i = 0; i < this.state.equipReservados.length; i++) {
      console.log("i " + i);
      for(var j = 1; j < this.state.equipReservados.length; j++) {
        console.log("j " + j);
      //&& tipo !== ""
      //alert("this.state.equipReservados[i].tipo: " + this.state.equipReservados[i].tipo + " tipo: " + tipo);
        if(this.state.equipReservados[i].familia === this.state.equipReservados[j].familia && (this.state.equipReservados[i].tipo === null) && (this.state.equipReservados[j].tipo == null)) {
    ...
    this.props.insertReserve(this.props.usuario, dateTimeReserve, this.state.materia, equips);
  }
