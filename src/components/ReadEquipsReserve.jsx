...
export default class ReadEquipsReserve extends React.Component {
  ...
  handleBlur(event) {
    ...
    if(this.state.fieldValueOnFocus != barCode.value
        || (this.state.fieldErrorCodeOnFocus == "" && this.state.fieldTipoOnFocus == null))
      this.props.getEquipTipo(id, Number(barCode.value));
    else if(this.state.fieldErrorCodeOnFocus != "") {
      this.props.setBarcodeErrorCode(id, this.state.fieldErrorCodeOnFocus);
      this.props.setBarcodeTipo(id, this.state.fieldTipoOnFocus);
    }
  }
