...
export default class ReadEquipsReserve extends React.Component {
  ...
  componentWillUpdate(nextProps) {
    ...
    nextProps.barCodes.forEach((newBarCode, newBarCodeIndex) => {
      if(newBarCode.oldIndex > 0 && !newBarCode.isLoading)
        this.props.resetOldIndex(newBarCodeIndex);
      let barCodeIndex;
      if(newBarCode.oldIndex == this.props.barCodes.length)
        barCodeIndex = newBarCode.oldIndex - 1;
      else if(newBarCode.oldIndex > 0)
        barCodeIndex = newBarCode.oldIndex;
      else
        barCodeIndex = newBarCodeIndex;

      const barCode = this.props.barCodes[barCodeIndex];
      ...
      if(barCode.errorCode == "ER_NOT_AVAILABLE") {}
        ...
        this.props.setBarcodeErrorCode(newBarCodeIndex, "ER_REPEATED_EQUIP");
      else if(newTipoIndexReserve !== null && newTipoIndexReserve >= 0) {
        this.props.incrementMissingReserve(newTipoIndexReserve);
        this.props.setBarcodeErrorCode(newBarCodeIndex, "");
        this.props.setIsPaper(newBarCodeIndex, true);
      }
        ...
        this.props.setBarcodeErrorCode(newBarCodeIndex, "ER_WRONG_EQUIP");
  ...
  handleRemoveBarCode(event) {
    ...
    if(removedEquipTipo !== null) {
      if(removedEquipErrorCode != 'ER_NOT_AVAILABLE'
        ...
        this.props.setBarcodeErrorCode(otherEquipIndex, "")
        this.props.setIsPaper(otherEquipIndex, true);
      }
    ...
    if(this.props.barCodes.length > 1) {
      ...
      this.props.setIsPaper(index, false)
    }
  }

  handleFocus(event) {
    const index = Number(event.currentTarget.name);
    this.props.setBarcodeErrorCode(index, "");
    this.props.clearBarcodeTipo(index);
    this.setState(update(this.state, {
      fieldValueOnFocus: {$set: this.props.barCodes[index].value},
      fieldErrorCodeOnFocus: {$set: this.props.barCodes[index].errorCode},
  ...
  handleBlur(event) {
    ...
    else if(this.state.fieldErrorCodeOnFocus != "") {
      this.props.setBarcodeErrorCode(index, this.state.fieldErrorCodeOnFocus);
                  ...
                  <Col xs={10}>
                    ...
                    {barCode.isPaper ?
                      ...
                      <TextField
                        ...
                        errorText={getErrorText(barCode.errorCode)}
...
ReadEquipsReserve.propTypes = {
  ...
  resetOldIndex: PropTypes.func.isRequired,
  setIsPaper: PropTypes.func.isRequired,
