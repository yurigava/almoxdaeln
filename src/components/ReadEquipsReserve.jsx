...
export default class ReadEquipsReserve extends React.Component {
  ...
  componentWillUpdate(nextProps) {
    ...
    nextProps.barCodes.forEach((newBarCode) => {
      ...
      if(barCode === undefined || barCode.equipTipo === newTipo || repeatedEquips.length > 1 && newBarCode.errorCode != "")
        return;
      ...
      else if(newBarCode.errorCode != "" && repeatedEquips.length > 1) {}//equip already exists in array
  ...
  handleBlur(event) {
    ...
    if(this.props.barCodes.filter(repBar => repBar.value == barCode.value).length > 1)
