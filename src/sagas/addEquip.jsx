...
function *insertEquips(action) {
  ...
  try {
    ...
    if(response.data === "ok") {
      ...
      yield put({ type: 'SET_SUBMISSION_MESSAGE', message: "Os equipamentos foram registrados com sucesso." });
    }
    else if(response.data.code === "ER_DUP_ENTRY") {
      yield put({ type: 'SET_DATA_SUBMITTED', submitted: false });
      yield put({ type: 'SET_SUBMISSION_MESSAGE', message: "Algum dos equipamentos já está registrado." });
      yield put({
